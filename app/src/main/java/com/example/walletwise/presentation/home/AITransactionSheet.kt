...
fun AITransactionSheet(viewModel: TransactionViewModel) {
    ...
    val busy = state.isAnalyzing || state.isSaving
    ...
    Column(Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        if (state.savedDraftId != null) Button(viewModel::newAIDraft) { Text("Giao dịch tiếp theo") }
        voiceMessage?.let {
            Text(it)
        }
        }
        AssistantComposer(input, { input = it }, !busy,
            ...
            onSend = { val submitted = input; input = ""; viewModel.processAITransaction(submitted) },
