...
final class TransactionMutationEmulatorProbe {
    ...
    private func exercise() async throws {
        ...
        let missing = WalletWiseShared.Transaction(id: "never-create-update", userId: created.userId, type: created.type, paymentMethod: created.paymentMethod, amount: created.amount, category: created.category, note: "", timestamp: created.timestamp, imageUrl: "", isLegacy: false)
    ...
    private func units() throws {
        ...
        let tx = WalletWiseShared.Transaction(id: "unit-doc", userId: uid, type: "Chi", paymentMethod: "Tiền mặt", amount: 50000, category: "Test", note: "", timestamp: 1700000000000, imageUrl: "", isLegacy: false)
        let add = IosTransactionWriteMapper.fields(kind: .add, transaction: tx)
        try check(Set(add.keys) == Set(["id", "userId", "type", "paymentMethod", "amount", "category", "note", "timestamp", "imageUrl"]) && add["amount"] is Double && add["timestamp"] is Int64 && add["imageUrl"] as? String == "", "native-add-wire-fields-and-types")
        ...
        try check(patch["id"] == nil && patch["userId"] == nil && patch["imageUrl"] == nil && patch.count == 6, "native-update-patch-preserves-other-fields")
