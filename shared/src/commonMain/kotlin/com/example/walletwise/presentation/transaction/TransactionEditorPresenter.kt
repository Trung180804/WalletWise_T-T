...
class TransactionEditorPresenter(
    ...
    fun submit() {
        ...
        val categoryId = availableCategories().firstOrNull { it.type == s.form.type && it.name == s.form.category }?.id
            ?: original?.takeIf { it.type == s.form.type && it.category == s.form.category }?.categoryId.orEmpty()
        val transaction = (original ?: Transaction()).copy(id = id, userId = uid, type = s.form.type, amount = amount,
            category = s.form.category, categoryId = categoryId, timestamp = s.form.timestamp, note = s.form.note, paymentMethod = s.form.paymentMethod)
